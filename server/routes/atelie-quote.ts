...
// Accepts Atelie form fields: name, phone, eventType, guests, date, time, details
export const handleAtelieQuote: RequestHandler = async (req, res) => {
  ...
  // Compose email payload for Atelie
  const quoteRequest = {
    brand: "atelie",
    fullName: name,
    whatsapp: phone,
    ...
    eventDate: date,
    eventTime: time,
    email: "atelie@dlm.ao", // or use a field if available
    message: details || "",
    requestId: `AT-${Date.now().toString(36).toUpperCase()}`,
